...
-- 평균 급여가 5000 보다 큰 부서의 정보를 조회하기
select DEPARTMENT_ID, ROUND(avg(SALARY)) 
from employees
-- where AVG(SALARY) > 5000 -- 전체 행에 대해서 평균 급여를 구함
...
-- 부서별 사원 수를 구하고 사원 수가 3명 이상인 부서만 조회하기
select DEPARTMENT_ID, count(*)
from employees
group by DEPARTMENT_ID
having count(*) >= 3;

-- 부서별 최고 급여가 10000이상인 부서 조회
...
-- 2025-01-02 이후의 거래만 대상으로, 카테고리별 총액이 2만원 이상인 경우만 조회하기
...
-- 카테고리별 총 판매 금액을 구하되, 판매금액이 200,000이상인 카테고리만 조회
select category, SUM(amount)
from sales
group by category
...
-- ROLLUP
-- GROUP BY가 그룹별로 집계를 해준다면,
-- ROLLUP은 소계와 총계까지 한 번에 만드는 기능
