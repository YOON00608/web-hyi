...
-- 각 상품의 가격을 3으로 나눈다.
select id, product, price, MOD(price, 3) from sales;

-- 모든 내용을 조회하되, 할인률은 %로 표기하기
select product,price,quantity, CONCAT(ceil(discount_rate * 100),'%') from sales;

-- 세일즈 테이블에서 아이디, 제품명, 가격, 재고, 총 가격(소수점 아래 버림)을 조회하기
select id, product, price, quantity, TRUNCATE(price*quantity,0)
from sales;
...
-- 상품 가격이 50000만보다 크면 1, 작으면 -1, 같으면 0이 되도록
-- 상품이름과 함께 조회하기
select product, SIGN(price - 50000)
from sales;

-- FORMAT()
-- 숫자를 사람이 보기 좋게 "문자열 형태로 포맷팅" 하는 함수
-- 특히 천 단위 콤마(,) + 소수점 자릿수 지정을 자동으로 해준다.
...
-- 2025년 1월에 주문된 주문만 조회하기
select * from orders
where year(order_date) = 2025
and MONTH(order_date) = 1;

-- 주문날짜 기준으로 2024년에 주문된 기록만 조회하기
select * from orders
where year(order_date) = 2024;

-- 주문날짜를 기준으로 작년에 주문된 기록만 조회하기
select * from orders
...
-- 사원테이블에서 이름, 급여를 출력하고 1000당 ■로 개수를 채워 조회하기
select FIRST_NAME, SALARY, repeat('■',round(SALARY/1000))
from employees;

-- 사원테이블에서 이름, 입사날짜, 6개월 뒤 입사날짜 순으로 조회하기
...
select count(*) from score; -- null을 포함한 개수
...
-- 평균 급여, 급여 최고액, 급여 최저액, 급여의 총 합계를 출력하기
...
-- 급여가 5000 이상인 사원들의 평균급여를 구하시오
select AVG(SALARY)
from employees
where SALARY >= 5000;

-- 2005년에 입사한 사원들의 수를 구하시오
select COUNT(*)
from employees
...
-- GROUP_BY 로 묶은 속성은 SELECT에서 집계함수와 함께 사용할 수 있다.
...
select FIRST_NAME, CHAR_LENGTH(FIRST_NAME),COUNT(*)
from employees
where FIRST_NAME like '%a%'
group by FIRST_NAME;


-- EMPLOYEES 테이블에서 직업(JOB_ID)별로 평균 급여를 구하세요.
...
group by DATE_FORMAT(HIRE_DATE, '%c월');

-- 사원 이름(FIRST_NAME)의 글자 수를 기준으로 그룹을 나누고,
-- 각 이름 길이별로 가장 높은 급여와 가장 낮은 급여를 조회하세요.
select char_length(FIRST_NAME), max(SALARY), MIN(SALARY)
from employees
group by char_length(FIRST_NAME);

-- EMPLOYEES 테이블에서 사원들의 성(LAST_NAME) 첫 글자를 추출하고, 
-- 각 알파벳별로 사원이 몇 명인지 조회하세요.
select left(last_name,1), count(*)
from employees
group by left(last_name,1);

-- EMPLOYEES 테이블에서 부서(DEPARTMENT_ID)별로 사원들의
-- 급여 총합과 평균 급여를 함께 조회하세요.
-- 단, 평균 급여는 소수점 아래를 버리고 정수로 표시하세요.
select DEPARTMENT_ID, sum(SALARY), FLOOR(AVG(SALARY))
from employees
group by DEPARTMENT_ID;

-- EMPLOYEES 테이블에서 급여(SALARY)가 10,000 이상인 사원들만 필터링한 뒤,
-- 각 직업(JOB_ID)별로 그런 사원이 몇 명인지 조회하세요.
