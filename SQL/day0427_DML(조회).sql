...
-- film 테이블에서 rental_rate가 2.99 이상인 영화의 제목과
-- rental_rate를 조회하세요.
...
-- customer 테이블에서 active 가 1인 고객만 조회하세요.
...
-- address 테이블에서 address2가 null인 주소를 조회하세요
select * from address
...
-- film 테이블에서 영화 길이가 60분 이하거나 180분 이상인 영화를 조회하세요
select title, length from film
where length <= 60 or length >= 180;

-- payment 테이블에서 2020년 1월 1일 이후에 결제된 내역을 조회하세요.
select * from payment
