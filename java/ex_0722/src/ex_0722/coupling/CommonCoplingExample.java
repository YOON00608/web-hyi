...
public class CommonCoplingExample {
	...
	static class ModuleA{
		public void updateDiscountRate() {
			GlobalSrate.globalDiscountRate = 20;
		}
	}
	
	// 가격 계산 모듈
	// ModuleA가 전역변수를 바꾼 파급효과가 ModuleB에 그대로 전달되어 얘기치 않은 문제가 발생할 수 있다.
	// 프로그램 전체에서 어느 모듈이 언제 전역 변수 값을 바꿨는지 추적하기가 힘들어 디버깅이 어려워진다.
	// ㅁ모듈 간 독립성이 사라져 특정 모듈만 떼어내어 재사용 하는 것이 불가능하다. 
	static class ModuleB{
		public void printPirce(int price) {
			int finalPrice = price - GlobalSrate.globalDiscountRate;
